...
class SettingsTab(Tab):
    ...
    def _draw_settings(self, parents):
        current_row = 0
    ...
    def _draw_settings_for_parent(self, parent_name, grid_row):
    ...
    def _create_parent_label(self, name, grid_row):
    ...
    def _create_label(self, container, text, column, row, pady=0):
        label = ctk.CTkLabel(container, text=text)
        label.grid(row=row, column=column, pady=pady, sticky=ctk.W)

    def _draw_option_setting(self, setting, grid_row):
    ...
    def _entry_changed(self, event, setting):
